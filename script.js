...
function filterType(type){
  ...
  filterDataRender(filterProducts);
}

function filterDataRender(filteredData){
...
function displayProducts(filteredData) {
  ...
  filteredData.forEach((product) => {
      list.innerHTML += `
          <div class="book-card" data-item="${product.type}">
              <div class="book-image">
                  <img src="https://picsum.photos/200/300?_random=${Math.floor(Math.random() * 100) + 1}" alt="Book Cover">
              </div>
              <div class="book-details">
                  <div class="book-type">${product.type}</div>
                  <div class="book-title">${product.title}</div>
                  <div class="book-author">${product.author}</div>
                  <div class="book-price">
                      <span class="book-price-symbol">$</span>${product.price}
                  </div>
                  <div class="buttons">
                      <button class="addToCart" onclick="addToCard(${product.id})">Add to cart</button>
                      <i class="ri-heart-line" id="heart"></i>
                      <i class='bx bx-dots-vertical-rounded options' onclick="option(${product.id})"></i>
                      <div class="inOption">
                          <i class='bx bx-pencil' onclick="renameItem(${product.id})"></i>
                          <i class="fa-solid fa-trash" onclick="removeItem(this)"></i>
                      </div>
                  </div>
              </div>
